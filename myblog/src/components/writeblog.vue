    ...
    <quill-editor id="editor"
      ...
      <div id="toolbar" slot="toolbar">
        ...
        <span class="ql-formats">
          ...
          <form action="./PHP/uploadImgs.php"
                enctype="multipart/form-data"
                method="post"
                id="upload-img-form">
            <input type="file" name="file" id="upload-img" accept="image/*" @change="selected"/>
...
<script>
  ...
  export default {
    ...
    data(){
        let content = '';
        if(this.$props.blog.blogTitle !== ''){
          content = this.$props.blog.blogHtml;
        }
        return {
          fileFlag:false,
          content,
    ...
    methods:{
      commit(){
          ...
          this.$store.commit('modifyEditorText',this.$refs.myTextEditor.quill.getText());
      },
      imgClick(){
      ...
      selected(){
          var uploadImg = document.getElementById('upload-img');
          var uploadImgForm = document.getElementById('upload-img-form');
          if(this.fileFlag === true){
              var fd = new FormData(uploadImgForm);
              var range = this.$refs.myTextEditor.quill.getSelection(true);
              var length = range.index;
              var that = this;
              this.$http.post('./PHP/uploadImgs.php',fd,{
                  emulateJSON:true,
              }).then((data)=>{
                    var imgUrl = './../PHP/UploadedPics/' + data.data + '.png';
                    that.$refs.myTextEditor.quill.insertEmbed(length, 'image', imgUrl);
                },
                (error)=>{
                  alert(error);
                });
